...
Lists are newest-first by default (descending id). `?sort=createdAt|updatedAt` sorts ascending, prefix `-` for descending. Sorted queries use an opaque composite cursor — pass `nextCursor` verbatim. Pagination is bidirectional: responses carry `nextCursor` and `prevCursor`; page back with `?direction=backward&cursor=<prevCursor>`.
